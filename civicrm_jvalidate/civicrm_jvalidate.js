  ...
  $.amask.phone_add_validate = function(obj, admin){
    ...
    if (admin) {
      ...
      if (typeof $(".contact_information-section").on !== 'undefined') {
        $(".contact_information-section").on("change", "select[name*='phone_type_id']", adminPhoneChange);
      }
      else {
        $(".contact_information-section select[name*='phone_type_id']").live("change", adminPhoneChange);
      }

      // phone input paste
      var adminPhoneFocus = function() {
        var type_id = Number($(this).closest('tr').find("select[name*='phone_type_id']").val());
        refreshMask(this, type_id);
      }

      if (typeof $(".contact_information-section").on !== 'undefined') {
        $(".contact_information-section").on("focus", "input[name$='[phone]']", adminPhoneFocus);
      }
      else {
        $(".contact_information-section input[name$='[phone]']").live("focus", adminPhoneFocus);
      }
    }
    else {
      // phone type change
